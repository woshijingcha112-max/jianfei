...
## 2026-04-15 / 收口智谱 API Key 配置方式

### 本轮完成内容

- 已将后端 [application.yml](/D:/source/sccp/codex/demo/backend/src/main/resources/application.yml) 中硬编码的智谱 `api-key` 改为环境变量占位 `${PHOTO_AI_ZHIPU_API_KEY:}`。
- 百度配置继续保持环境变量读取方式，不新增本地明文凭证文件入库。

### 本轮验证

- 已执行定向搜索，确认源码配置层不再保留硬编码智谱 key。

### 备注

- 历史上进入过仓库的旧 key 应按已泄漏处理，需在平台侧作废并换新。
